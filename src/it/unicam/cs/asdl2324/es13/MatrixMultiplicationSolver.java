...
public class MatrixMultiplicationSolver {
    ...
    private void solve() {
        ...
        for(int len=2; len<this.p.size();len++) {	//Calcoliamo i costi minimi per le sotto-sequenze di lunghezza crescente
            for(int i=0;i<this.p.size()-len;i++) {
                int j=i+len-1;
                
                this.m[i][j]=Integer.MAX_VALUE;	//Inizializziamo il costo minimo con il valore più alto possibile
                
                for(int k=i;k<j;k++) {	//Cerchiamo il valore di k che minimizza il costo per la moltiplicazione di A[i]...A[k] e A[k+1]...A[j]
                    
                	//Calcoliamo il costo per la moltiplicazione di A[i]...A[k] e A[k+1]...A[j] e
                    int cost=this.m[i][k]+this.m[k+1][j]+this.p.get(i)*this.p.get(k+1)*this.p.get(j+1);
                    if(cost<this.m[i][j]) {	//Verifichiamo se il costo è minore del costo minimo attuale
                        this.m[i][j]=cost;	// Aggiorniamo il costo minimo
                        this.b[i][j] = k;	// Aggiorniamo la scelta di k
                    }
                }
            }
        }
	...
	private String traceBack(int i, int j) {
        ...
        if (i==j)	//Se i e j sono uguali, la sottosequenza è una sola matrice, quindi la stampiamo e terminiamo la ricorsione
            return "A_{" +i+ "}";
